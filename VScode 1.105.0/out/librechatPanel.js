...
class LibreChatPanel {
    static createOrShow(extensionUri) {
        ...
        const panel = vscode.window.createWebviewPanel('librechat', 'LibreChat Assistant', column || vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    async _handleSendMessage(message) {
        try {
            ...
            const autoAttach = config.get('autoAttachContext');
            if (autoAttach) {
                context = await this._getCurrentFileContext();
            }
            // Send message to LibreChat
            const response = await librechatClient_1.LibreChatClient.sendMessage(message, context);
    ...
    async _handleExplainCode(code) {
        try {
            const response = await librechatClient_1.LibreChatClient.sendMessage(`Please explain this code:\n\n${code}`, {});
            this._panel.webview.postMessage({
    ...
    async _getCurrentFileContext() {
        ...
        const config = vscode.workspace.getConfiguration('librechatAssistant');
        const maxLines = config.get('maxContextLines');
    ...
    static async explainCode(code, extensionUri) {
    ...
    static async debugCode(code, extensionUri, diagnostics) {
        this.createOrShow(extensionUri);
        setTimeout(() => {
            if (this.currentPanel) {
                let message = `Please help me debug this code:\n\n${code}`;
                if (diagnostics && diagnostics.length > 0) {
                    const diagnosticMessages = diagnostics.map(d => `Line ${d.range.start.line + 1}: ${d.message}`).join('\n');
                    message += `\n\nCurrent errors:\n${diagnosticMessages}`;
                }
                this.currentPanel._panel.webview.postMessage({
                    type: 'sendMessage',
    ...
    static async optimizeCode(code, extensionUri) {
        this.createOrShow(extensionUri);
        setTimeout(() => {
            if (this.currentPanel) {
                this.currentPanel._panel.webview.postMessage({
...
exports.LibreChatPanel = LibreChatPanel;
function getNonce() {
    let text = '';
...
//# sourceMappingURL=librechatPanel.js.map